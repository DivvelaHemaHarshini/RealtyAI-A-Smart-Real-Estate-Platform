...
def predict_and_classify_image_from_bytes(
    bytes_data,
    model,
    ...
    """
    Args:
        bytes_data: image bytes (from uploaded_file.read()).
        model: loaded segmentation model with .predict()
        model_input_size: (H, W) used if model.input_shape is not available
        area_thresh: pixel-area threshold to mark buildings 'large'

    Returns:
        mask (H, W, 3 uint8)         - binary mask (0/255 RGB)
        img_type (str)               - classification string
        avg_area (float)             - average building area (px)
        max_area (float)             - max building area (px)
        n_buildings (int)            - number of detected building components
        overlay (H, W, 3 uint8)      - original image with mask overlay
    """
    ...
    # 2) Determine target_shape for preprocess_image
    # Prefer model.input_shape if present and sensible, else build one from model_input_size
    try:
        inp_shape = model.input_shape  # e.g., (None, H, W, C)
        if inp_shape is None or len(inp_shape) < 4:
            raise Exception("bad input_shape")
        target_shape = (None, inp_shape[1], inp_shape[2], inp_shape[3])
    except Exception:
        # fallback: assume 3 channels
        target_shape = (None, model_input_size[0], model_input_size[1], 3)

    # 3) Preprocess & predict
    processed = preprocess_image(image_pil, target_shape)
    pred_raw = model.predict(processed)  # model output shape assumed like (1,H,W,1) or (1,H,W,3)
    mask_pred = post_process_mask(pred_raw)  # returns an RGB mask (0/255)

    # 4) Resize mask back to original image size (nearest to keep binary)
    mask_resized = cv2.resize(mask_pred, (orig_w, orig_h), interpolation=cv2.INTER_NEAREST)

    # 5) Convert mask to single-channel binary (0/255)
    if mask_resized.ndim == 3:
        # If mask is RGB, consider any channel > 127 as mask
        mask_gray = cv2.cvtColor(mask_resized, cv2.COLOR_RGB2GRAY)
    else:
        mask_gray = mask_resized.copy()

    _, mask_bin = cv2.threshold(mask_gray, 127, 255, cv2.THRESH_BINARY)

    # 6) Find connected components / contours to compute areas
    contours, _ = cv2.findContours(mask_bin, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    areas = [cv2.contourArea(c) for c in contours if cv2.contourArea(c) >= 1.0]

    n_buildings = len(areas)
    if n_buildings > 0:
    ...
    # 7) Basic classification logic (zoning intensity) — same approach as earlier snippet
    # density = fraction of pixels predicted as building * 100 (percentage)
    density = (np.sum(mask_bin > 0) / (mask_bin.shape[0] * mask_bin.shape[1])) * 100.0
    RES_MAX, COM_START = 15.0, 10.0
    res_score = (density / RES_MAX) * 100.0 if density <= RES_MAX else 100.0
    com_score = ((density - COM_START) / 35.0) * 100.0 if density > COM_START else 0.0
    com_score = min(max(com_score, 0.0), 100.0)

    # Compose a final classification label using both density and building sizes
    cls = "Open Area"
    if res_score > 20:
        cls = "Residential Zone"
    if com_score > 20:
        cls = "Commercial / Mixed"

    # Optionally mark "Large" if many large connected components exist
    n_large = sum(1 for a in areas if a >= area_thresh)
    if n_large > max(1, 0.03 * max(1, n_buildings)):  # heuristic: a few large buildings -> mixed/industrial
        cls = "Commercial / Mixed" if cls == "Residential Zone" else cls

    # 8) Create overlay: blend red mask onto original
    # mask_bin is single-channel 0/255. Build 3-channel colored mask and alpha-blend
    colored_mask = np.zeros_like(orig_np, dtype=np.uint8)
    # Red color where mask==255
    colored_mask[mask_bin == 255] = (220, 30, 30)  # red-ish

    # alpha: stronger where mask exists, else 0
    alpha = (mask_bin.astype(np.float32) / 255.0) * 0.45  # 0.0-0.45 alpha
    alpha_3 = np.stack([alpha, alpha, alpha], axis=2)

    overlay = (orig_np.astype(np.float32) * (1 - alpha_3) + colored_mask.astype(np.float32) * alpha_3)
    overlay = np.clip(overlay, 0, 255).astype(np.uint8)

    # 9) Compose a 3-channel binary mask for output (so st.image works)
    mask_rgb_out = cv2.cvtColor(mask_bin, cv2.COLOR_GRAY2RGB)

    return mask_rgb_out, cls, avg_area, max_area, n_buildings, overlay
...
spacenet_model = load_unet_model()
...
""", unsafe_allow_html=True)
    ...
    # ===============================
        ...
        """, unsafe_allow_html=True)
            ...
            st.header("🛰️ Satellite Image Segmentation & Classification")
            st.caption("Upload satellite imagery for building detection and zoning intensity estimation.")

            # Load model (use your own function)
            ...
            # Session object
            if "satellite_res" not in st.session_state:
                st.session_state.satellite_res = None

            # Main Processing
            if uploaded_file:
                file_key = f"{uploaded_file.name}_{uploaded_file.size}"

                # Recompute if new file uploaded
                if (
                    st.session_state.satellite_res is None
                    or st.session_state.satellite_res["id"] != file_key
                ):
                    with st.spinner("🔍 Analysing satellite image..."):
                        ...
                        # Preprocessing adapted to your second-snippet logic
                        processed = preprocess_image(image_pil, model.input_shape)

                        # Predict
                        mask_raw = model.predict(processed)
                        mask_img = post_process_mask(mask_raw)  # binary
                    
                        # Apply your original classification logic
                        (
                            bin_mask,
                            img_type,
                            avg_area,
                            ...
                            n_buildings,
                            overlay
                        ) = predict_and_classify_image_from_bytes(
                            bytes_data,
                            model,
                            model_input_size=(128, 128),
                        ...
                        # Buffer conversions
                        is_success, buf_mask = cv2.imencode(".png", bin_mask.astype("uint8"))
                        is_success, buf_overlay = cv2.imencode(".png", overlay)

                        st.session_state.satellite_res = {
                            "id": file_key,
                            "img": image_pil,
                            "mask": bin_mask,
                            "mask_bytes": io.BytesIO(buf_mask).getvalue(),
                            "overlay": overlay,
                            "overlay_bytes": io.BytesIO(buf_overlay).getvalue(),
                    ...
                    # Display Results
                    res = st.session_state.satellite_res
                    # if res:

                     # Beautiful formatted card
                    components.html(f"""
                    <div style='background:#f9fbe7; padding:22px; border-radius:12px;
                                    box-shadow:0 2px 6px rgba(0,0,0,0.08); margin-top:10px;'>

                            <h2 style='color:#1967d2; text-align:center; margin-bottom:16px;'>
                                🏠 Classification Result
                            </h2>

                            <div style='font-size:20px; text-align:center; margin-bottom:8px;'>
                                <b style='color:#2e7d32;'>Detected Type:</b>
                                <span style='color:#333;'>{res["classification"]}</span>
                            </div>

                            <div style='font-size:18px; text-align:center;'>
                                <b>Total Buildings:</b> {res["n_buildings"]}
                            </div>

                            <div style='font-size:18px; text-align:center;'>
                                <b>Average Area:</b> {res["avg_area"]:.2f} sq px
                            </div>

                            <div style='font-size:18px; text-align:center;'>
                                <b>Max Area:</b> {res["max_area"]:.2f} sq px
                            </div>
                    ...
                    st.session_state.classified = True  # Set flag for house prediction
                    st.session_state.classification_result = res["classification"]
...
st.write("RealtyAI-A Smart Real estate Platform"|Powered By HarshiniDivvela|©️2025")
